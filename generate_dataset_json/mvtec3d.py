...
class MVTec3DSolver(object):
    CLSNAMES = [
        'bagel', 'carrot', 'cable_gland', 'cookie', 'dowel',
        'foam', 'peach', 'potato', 'rope', 'tire',
    ]

    def __init__(self, root='data/mvtec3d'):
        self.root = root
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate MVTec3D dataset JSON metadata')
    parser.add_argument('--root', type=str, required=True,
                        help='Root directory of the MVTec3D dataset')
    args = parser.parse_args()
    runner = MVTec3DSolver(root=args.root)
    runner.run()
